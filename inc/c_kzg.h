 ...
 *
 * @warning In the case of @p CKzgOk or @p CKzgBadargs, the caller can assume that all memory allocated by the
 * called routines has been deallocated. However, in the case of @p CKzgError or @p CKzgMalloc being returned, these
 * are unrecoverable and memory may have been leaked.
 *
 * @todo Check that memory is not leaked anywhere in the case of CKzgBadargs.
 */
typedef enum {
    C_KZG_OK = 0,  /**< Success! */
