...
const EXPECTED_POWERS: [[u64; 4usize]; 11] = [
    ...
    [
        17407663719861420663,
        10645919139951883969,
        123407966953127,
        0,
    ],
    [
    ...
    [
        4160126872399834567,
        5941227867469556516,
        11658769961926678707,
        7254684264,
    ],
    [
        4000187329613806065,
        4317886535621327299,
        17988956659770583631,
        238899937640724696,
    ],
];
...
pub fn evaluate_polynomial_in_evaluation_form_test<
    TFr: Fr,
    TG1: G1,
    TG2: G2,
    TPoly: Poly<TFr>,
...
>(
    ...
    let lvals: [[u64; 4]; 4] = [
        ...
        [
            281474976710667,
            17006436628450967565,
            10183145419576640720,
            0,
        ], // 3465144826073652318776269530687742778510060141723586134027
        [
            18446462594437873676,
            7474466853796666379,
            11954817552772682548,
            8353516859464449351,
...
pub fn compute_commitment_for_blobs_test<
    TFr: Fr,
    TG1: G1,
    TG2: G2,
    TPoly: Poly<TFr>,
...
>(
    ...
    let mut blobs_sedes: ssz_rs::List<ssz_rs::Vector<[u8; 32], BLOB_SIZE>, MAX_BLOBS_PER_BLOCK> =
        ssz_rs::List::default();
    for _ in 0..3 {
        let mut vec = Vec::new();
    ...
    let mut kzg_commitments_sedes: ssz_rs::List<ssz_rs::Vector<u8, 48>, MAX_BLOBS_PER_BLOCK> =
        ssz_rs::List::default();
    ...
    assert_eq!(
        bytes_from_g1(&simple_commitment),
        bytes_from_g1(&aggregated_poly_commitment)
    );

    assert!(
        verify_kzg_proof(&simple_commitment, &x, &y, &proof, &ts),
        "Simple verification failed"
    );

    assert!(
        verify_kzg_proof(&aggregated_poly_commitment, &x, &y, &proof, &ts),
        "Verification failed"
    );
    ...
    assert!(
        !verify_kzg_proof(&aggregated_poly_commitment, &x2, &y2, &proof, &ts),
...
>(
    ...
    assert!(
        verify_aggregate_kzg_proof(&blobs, &kzg_commitments, &proof, &ts),
        "verify failed"
    );
...
>(
    ...
    let expected_y = TFr::from_u64_arr(&[
        222763632366299915,
        460490682938831241,
        4083331670220749024,
        8059712683068040737,
    ...
    let root_of_unity: TFr = TFr::from_u64_arr(&[
        16286944871763370758,
        779461914329595798,
        18176117771551122527,
        6218356256323077364,
    ]);
    let roots_of_unity = (0..BLOB_SIZE as usize)
        .map(|i| root_of_unity.pow(i))
        .collect::<Vec<TFr>>();

    let mut polynomial_l = roots_of_unity
        .iter()
        .map(|w| polynomial.eval(w))
        .collect::<Vec<TFr>>();
    ...
    let expected_bytes: [u8; 48] = [
        0x80, 0x80, 0x90, 0xf8, 0x46, 0x7c, 0xd, 0x83, 0xdc, 0xf5, 0x4e, 0x82, 0x52, 0xcd, 0xd5,
        0x46, 0xeb, 0x2f, 0xcb, 0xab, 0xbb, 0x14, 0x3a, 0x8e, 0xf1, 0xb1, 0xf8, 0x96, 0x3b, 0xc,
        0xd8, 0x7e, 0xe7, 0x4e, 0xc8, 0x2e, 0xc3, 0x5d, 0x85, 0x59, 0x2d, 0x16, 0xb0, 0xfc, 0x8e,
        0xa1, 0x70, 0x8e,
    ];
...
>(
    ...
    assert!(
        verify_aggregate_kzg_proof(&[], &[], &compute_aggregate_kzg_proof(&[], &ts), &ts),
        "verify failed"
    );
}
