...
pub fn roots_of_unity_out_of_bounds_fails<Fr: TFr, FFTSettings: TFFTSettings<Fr>>() {
    let fft_settings = FFTSettings::new(32);
    assert!(fft_settings.is_err());
}

/// Raise each root to the power of 2 ^ i and see if it equals 1
pub fn roots_of_unity_are_plausible<Fr: TFr>(roots: &[[u64; 4]; 32]) {
    for i in 0..32 {
        let mut r = Fr::from_u64_arr(&roots[i]);
        for _j in 0..i {
            r = r.sqr();
        }
...
pub fn expand_roots_is_plausible<Fr: TFr>(roots: &[[u64; 4]; 32], expand_root_of_unity: &dyn Fn(&Fr, usize) -> Result<Vec<Fr>, String>) {
    let scale = 15;
    let width: usize = 1 << scale;

    let root = Fr::from_u64_arr(&roots[scale]);
...
pub fn new_fft_settings_is_plausible<Fr: TFr, FFTSettings: TFFTSettings<Fr>>() {
    let scale = 21;
    let width: usize = 1 << scale;
