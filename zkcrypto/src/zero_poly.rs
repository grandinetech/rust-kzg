...
impl ZeroPoly<blsScalar, ZPoly> for ZkFFTSettings {
	...
	-> Result<ZPoly, String> {
        ...
        if out_degree + 1 > length {
            return Err(String::from("Expected out_degree to be within possible polynomial size"));
        }
	...
	-> Result<(Vec<blsScalar>, ZPoly), String> {
        ...
        if missing_indices.len() >= length {
            ...
            return Err(String::from("Domain size is greater than fft_settings.max_width") );
        } 
        ...
        if missing_indices.len() <= missing_per_partial {
            ...
            for _i in 0..partial_count {
                ...
                let mut partial = self.do_zero_poly_mul_partial(&missing_indices[offset..end], domain_stride)?;
