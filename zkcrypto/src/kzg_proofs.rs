...
pub(crate) fn compute_proof_multi(p: &Poly, x: &Scalar, n: usize, ks: &KZGSettings) -> Result<G1, String> {    
...
pub(crate) fn check_proof_multi(com: &G1, proof: &G1, x: &Scalar, values: &Vec<Scalar>, n: usize, ks: &KZGSettings) -> Result<bool, String> {
    ...
    let mut poly = Poly { coeffs: ks.fs.fft_fr(values, true)? };
