...
// pub(crate) fn do_zero_poly_mul_partial(
//     fs: &FFTSettings,
//     indices: &[usize],
//     stride: usize,
// ) -> Result<PolyData, Error> {
//     if indices.len() == 0 {
...
//     poly.coeffs[0] = blst_fr_into_pc_fr(&fs.expanded_roots_of_unity[indices[0] * stride]).neg();

//     for i in 1..indices.len() {
//         let neg_di = blst_fr_into_pc_fr(&fs.expanded_roots_of_unity[indices[i] * stride]).neg();
...
//     let mut mul_eval_ps = fft_fr(&p_partial, false, fs).unwrap();

//     for i in 1..(partials.len()) {
//         p_partial = pad_poly(&partials[i], len_out)?;

//         let p_eval = fft_fr(&p_partial, false, &fs).unwrap();
//         for j in 0..len_out {
...
//     let coeffs = fft_fr(&mul_eval_ps, true, fs)?;
...
// pub(crate) fn from_u64_arr(x: &[u64; 4]) -> BlstFr {
//     BlstFr::from_u64_arr(&x.clone())
// }

// pub(crate) fn fr_is_zero(n: BlstFr) -> bool {
//     n.0.l[0] == 0 && n.0.l[1] == 0 && n.0.l[2] == 0 && n.0.l[3] == 0
// }

//Partially taken from team 'From-scratch'
// pub(crate) fn zero_poly_via_multiplication(
//     fs: &FFTSettings,
//     length: usize,
...
//     } else if length > fs.max_width as usize {
...
//     let domain_stride = fs.max_width as usize / length;
//     let mut partial_count = (missing_per_partial + missing_indices.len() - 1) / missing_per_partial;
...
//         zero_poly = do_zero_poly_mul_partial(&fs, &missing_indices, domain_stride)?;
//     } else {
//         let mut work = vec![BlstFr::zero(); (partial_count * degree_of_partial).next_power_of_two()];
...
//             let mut partial =
//                 do_zero_poly_mul_partial(&fs, &missing_indices[offset..end], domain_stride)?;
...
//                 if partials_num > 1 {
//                     let mut reduced_poly = reduce_partials(&fs, reduced_len, &partial_vec)?;
...
//     zero_eval = fft_fr(&zero_poly.coeffs, false, &fs)?;
