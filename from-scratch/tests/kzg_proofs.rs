...
mod tests {
    ...
    use kzg::{Fr, G1, G2, FFTSettings, KZGSettings};
    ...
    fn proof_multi() {
        ...
        let fs1 = FsFFTSettings::new(coeffs.len()).unwrap();
        let fs2 = FsFFTSettings::new(coeffs.len()).unwrap();

        //FsKZGSettings ks1, ks2;
        let kzgSettings_1 = KZGSettings::new(coeffs.len()).unwrap();
        let kzgSettings_2 = KZGSettings::new(coeffs.len()).unwrap();

        let mut p = FsPoly { coeffs: vec![Fr::default(); 16]};
        
        // g1_t commitment, proof;
        let commitment: G1 = G1::default();
        //let proof: G1;
        ...
        let fs1 = FFTSettings::new(4);

        // TEST_CHECK(C_KZG_OK == new_kzg_settings(&kzgSettings_1, s1, s2, secrets_len, &fs1));
        let kzgSettings_1 = KZGSettings::new(s1, s2, secrets_len, &fs1);
        ...
        let fs2 = FFTSettings::new(coset_scale);

        // TEST_CHECK(C_KZG_OK == new_kzg_settings(&ks2, s1, s2, secrets_len, &fs2));
        let kzgSettings_2 = KZGSettings::new(s1, s2, secrets_len, &fs2);
        ...
        let result = compute_proof_multi(&p, &x, coset_len, &kzgSettings_2);
        assert!(result.is_ok());
        ...
        for i in 0..coset_len {
            ...
            //eval_poly(&y[i], &p, &tmp);
            tmp = x.mul(&kzgSettings_2.fs.expanded_roots_of_unity[i]);
            y[i] = p.eval(&tmp);
        }
        ...
        let result = check_proof_multi(&commitment, &proof, &x, y, coset_len, &kzgSettings_2); //return through params mayb?
        assert!(result.is_ok());
